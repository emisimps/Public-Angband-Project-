...
#include "z-form.h"
#include <stdint.h> //SG
#include <stdbool.h> //SG
#include "player.h" //SG player.c needs to struct player in player.h to modify fields (totalPlayTime)
#include <stddef.h> //SG 
...
//SG Monotonic milliseconds, wraps safely via uint32_t */
static uint32_t monotonic_ms(void)
{
 ...
 //SG
uint32_t player_monotonic_ms(void)
{
	return monotonic_ms();
}

//SG Safe elapsed even across uint32_t wraparound */
...
void initializeSession(struct player *p)
{
	if (!p) return;
	p->total_play_time_ms = 0;
	p->session_start_ms = 0;
	p->session_running = false;
}

void startSession(struct player *p)
{
	if (!p) return;
	if (p->session_running) return;

	p->session_start_ms = monotonic_ms();
	p->session_running = true;
}

void endSession(struct player *p)
{
	if (!p) return;
	if (!p->session_running) return;

	uint32_t now = monotonic_ms();
	p->total_play_time_ms += (uint64_t)elapsed_ms(p->session_start_ms, now);
	p->session_running = false;
}

void Timer(struct player *p)
{
	if (!p) return;
	if (!p->session_running) return;
	...
	uint32_t now = monotonic_ms();
	p->total_play_time_ms += (uint64_t)elapsed_ms(p->session_start_ms, now);
	p->session_start_ms = now;
}

uint64_t getTotalPlayTime(const struct player *p)
{
	if (!p) return 0;

	uint64_t total = p->total_play_time_ms;

	if (p->session_running) {
		uint32_t now = monotonic_ms();
		total += (uint64_t)elapsed_ms(p->session_start_ms, now);
	}

	return total;
}

void format_playtime(uint64_t ms, char *buf, size_t n)
{
