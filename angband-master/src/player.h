...
#include "option.h"
#include <stddef.h>
#include <stdint.h> //SG gives us large positive numbers and needed for uint_t total_play_seconds
#include <stdbool.h> //SG needed for session_running (bool) to determine if game is active or not
...
struct player {
	...
	uint64_t total_play_time_ms; //SG accumulated play time in milliseconds
	uint32_t session_start_ms; //SG monotonic ms timestamp when session started
	bool session_running; //SG is the timer currently running?
...
uint32_t player_monotonic_ms(void); //SG
