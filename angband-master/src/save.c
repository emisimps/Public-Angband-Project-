...
void wr_player(void)
{
	...
	//SG Future use (32 bytes total). Store total_play_time_ms (8 bytes) + padding (24 bytes) */
	{
		uint32_t lo = (uint32_t)(player->total_play_time_ms & 0xFFFFFFFFu); //SG lower 32 bits of saved 64-bit time
		uint32_t hi = (uint32_t)(player->total_play_time_ms >> 32); //SG upper 32 bits of saved 64-bit time

		wr_u32b(lo); //SG 4 bytes
		wr_u32b(hi); //SG 4 bytes

		for (i = 0; i < 6; i++) {
			wr_u32b(0); //SG 6 * 4 = 24 bytes
		}
	}
}
