...
static struct panel *get_panel_misc(void) {
	...
	struct panel *p = panel_allocate(8); //SG added one line from 7
	...
	//SG show the total playtime
	format_playtime(t, tbuf, sizeof(tbuf));
	panel_line(p, attr, "Play Time", "%s", tbuf);
...
} panels[] =
{
	...
	{ { 21, 1, 18, 8 }, false, get_panel_misc },	/* Age, ht, wt, ... */ //SG 4th pos to 8
...
void do_cmd_change_name(void)
{
	...
	while (more) {
		...
		if (ke.type == EVT_NONE) {
			/* No key pressed — just loop and redraw again */
			continue;
		}
