...
static game_event_type statusline_events[] =
{
	...
	EVENT_FEELING,
	EVENT_LIGHT,
	EVENT_END, //SG occurs at the end of users turn, timer will increment as player moves and rests
};
...
//SG printer function for time
static size_t prt_time(int row, int col)
{
    char tbuf[32];
    char out[64];

    uint64_t t = getTotalPlayTime(player);

    format_playtime(t, tbuf, sizeof(tbuf));
    strnfmt(out, sizeof(out), "Time %s ", tbuf);

    c_put_str(COLOUR_L_GREEN, out, row, col);
    return strlen(out);
}
...
{ prt_level_feeling, prt_light, prt_time, prt_moves, prt_unignore, prt_recall,
...
{
    static uint32_t last_timer_redraw_ms = 0;

    //SG Ensures timer is updated even when character is idle
    {
        uint32_t now;
    #ifdef WINDOWS
        ...
        now = (uint32_t)(ts.tv_sec * 1000u + (uint32_t)(ts.tv_nsec / 1000000u));
    #endif

        if ((uint32_t)(now - last_timer_redraw_ms) >= 250u) {
            last_timer_redraw_ms = now;
    ...
    //SG animate and redraw
    do_animation();
    redraw_stuff(player);

    //SG Refresh the main screen
    Term_fresh();
}
