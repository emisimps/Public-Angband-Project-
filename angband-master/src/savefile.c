...
#include "z-file.h"
extern int rd_player_v1(void);
extern int rd_player_v2(void);
/**
 * The savefile code.
 *
...
static const struct blockinfo loaders[] = {
	...
	{ "quests", rd_quests, 1 },
	{ "player", rd_player_v1, 1 },
	{ "player", rd_player_v2, 2 },
	{ "ignore", rd_ignore, 1 },
	{ "misc", rd_misc, 1 },
...
bool savefile_save(const char *path)
{
	...
	if (file) {
		endSession(player); //SG stops counting the play-time when saving the game
		...
		startSession(player); //SG resumes timer counter after saving the game
		file_close(file);
	} else {
		character_saved = false;
	}
...
bool savefile_load(const char *path, bool cheat_death)
{
	...
	startSession(player); //SG starts timer after whole save finishes loading
