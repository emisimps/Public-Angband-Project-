...
/* SG: Future use (32 bytes total) */

	player->total_play_time_ms = 0; //SG vesioning bug
	player->session_running = false; //SG forces timer to be marked as non-running
	player->session_start_ms = 0; //SG clears old timestamp 
...
{
    int err = rd_player_v1();
    if (err) return err;

    //SG read total_play_time_ms (8 bytes) + reserved padding (24 bytes)
    uint32_t lo = 0, hi = 0;
    rd_u32b(&lo);
    rd_u32b(&hi);
    player->total_play_time_ms = ((uint64_t)hi << 32) | (uint64_t)lo;
    ...
    player->session_start_ms = 0;
    player->session_running = false;
